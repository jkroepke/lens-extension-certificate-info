...
export class SecretDetails extends React.Component<Component.KubeObjectDetailsProps<K8sApi.Secret>> {
  formatDate(dateString: string) {
    ...
    return (
      <span color={color}>{dateString} <i>(Valid: {diffInDays} days)</i></span>
    )
  }

  formatSAN(subjectaltname: string) {
    return subjectaltname?.split(/, /).map(s=><React.Fragment>{s}<br/></React.Fragment>)
  }
  ...
  render() {
    ...
    for (const key of secretKeys) {
      const certificateString = Buffer.from(this.props.object.data[key], 'base64').toString('ascii');

      if (!certificateString.startsWith("-----BEGIN CERTIFICATE-----")) continue;
